...
WHERE SC.Sno IS NULL;  -- 右表为 NULL 说明没匹配
```
...
SELECT C.Cno, C.Cname, SC.Sno, SC.Grade
FROM Course C          -- 换到左边
LEFT JOIN SC ON SC.Cno = C.Cno;  -- SC 换到右边
```

**结果完全相同！** 实际开发中，大家更习惯用 LEFT JOIN，所以 RIGHT JOIN 用得较少。
...
FROM SC
RIGHT JOIN Course C ON SC.Cno = C.Cno
WHERE SC.Cno IS NULL;  -- 左表为 NULL 说明没匹配
```
...
**⚠️ 重要：MySQL 不支持 FULL OUTER JOIN！** 需要用 UNION 模拟。
...
WHERE S.Sno IS NULL;  -- 只要第一部分没有的（没匹配的课程）
```
...
```mermaid
graph TB
    subgraph "第一部分: LEFT JOIN Student"
    A[Student S001-S004] --> B[包含赵六]
    end
    
    subgraph "第二部分: LEFT JOIN Course"
    C[Course C001-C005] --> D[包含C004,C005]
    end
    
    B --> E[UNION 合并]
    D --> E
    E --> F[完整结果<br/>学生+课程都保留]
